...
def _to_torch(x, env):
    """Convert IsaacLab 3.0 / Warp-backed sensor/object arrays to torch tensors."""
...
def object_root_pose(
    env: ManagerBasedRLEnv,
    asset_cfg: SceneEntityCfg,
) -> torch.Tensor:
    asset: RigidObject = env.scene[asset_cfg.name]
    return torch.cat([_to_torch(asset.data.root_pos_w, env), _to_torch(asset.data.root_quat_w, env)], dim=-1)


def cube_in_box(
    env: ManagerBasedRLEnv,
    cube_cfg: SceneEntityCfg,
    box_cfg: SceneEntityCfg | None = None,
    box_center: tuple[float, float, float] = (0.24, 0.16, 0.045),
    center_offset: tuple[float, float, float] = (0.0, 0.0, 0.0),
    half_extents: tuple[float, float, float] = (0.065, 0.065, 0.055),
    warmup_steps: int = 30,
) -> torch.Tensor:
    cube: RigidObject = env.scene[cube_cfg.name]
    cube_pos = _to_torch(cube.data.root_pos_w, env) - env.scene.env_origins.to(device=env.device, dtype=torch.float32)
    if box_cfg is None:
        center = torch.tensor(box_center, device=env.device, dtype=torch.float32)
    else:
        box: RigidObject = env.scene[box_cfg.name]
        center = _to_torch(box.data.root_pos_w, env) - env.scene.env_origins.to(device=env.device, dtype=torch.float32)
        center = center + torch.tensor(center_offset, device=env.device, dtype=torch.float32)
    half = torch.tensor(half_extents, device=env.device, dtype=torch.float32)
    lower = center - half
    upper = center + half
    inside = torch.all((cube_pos >= lower) & (cube_pos <= upper), dim=-1)
    settled = env.episode_length_buf >= warmup_steps
    return (inside & settled).unsqueeze(-1)


def cube_in_box_termination(
    env: ManagerBasedRLEnv,
    cube_cfg: SceneEntityCfg,
    box_cfg: SceneEntityCfg | None = None,
    box_center: tuple[float, float, float] = (0.24, 0.16, 0.045),
    ...
    return cube_in_box(env, cube_cfg, box_cfg, box_center, center_offset, half_extents, warmup_steps).squeeze(-1)
...
def any_vial_grasped(
    ...
    contact_forces = _to_torch(contact_sensor.data.force_matrix_w, env)
    ...
    for vial_idx, vial_name in enumerate(vials):
        ...
        vial_z = _to_torch(vial.data.root_pos_w, env)[:, 2]  # Z position per environment
...
def vial_placed_on_rack(
    ...
    rack_pos_w = _to_torch(rack_obj.data.root_pos_w, env)       # (num_envs, 3)
    rack_quat_w = _to_torch(rack_obj.data.root_quat_w, env)     # (num_envs, 4)
    ...
    for vial_idx, vial_name in enumerate(vials):
        vial: RigidObject = env.scene[vial_name]
        vial_pos_w = _to_torch(vial.data.root_pos_w, env)       # (num_envs, 3)
        vial_quat_w = _to_torch(vial.data.root_quat_w, env)     # (num_envs, 4)
...
def vial_placed_on_rack_termination(
    ...
    if env._rack_confirm_active.any():
        ...
        rack_obj: RigidObject = env.scene[rack_name]
        rack_pos_w = _to_torch(rack_obj.data.root_pos_w, env)
        rack_quat_w = _to_torch(rack_obj.data.root_quat_w, env)
        rack_quat_inv = math_utils.quat_inv(rack_quat_w)
        ...
        for vial_idx, vial_name in enumerate(vials):
            ...
            vial_pos_w = _to_torch(vial_obj.data.root_pos_w, env)
            vial_quat_w = _to_torch(vial_obj.data.root_quat_w, env)
