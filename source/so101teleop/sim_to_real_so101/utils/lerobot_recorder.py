...
class LeRobotRecorder:
    ...
    def __init__(
        ...
        self.initial_total_episodes = 0

        self.device = device
        self.capcity = max(1, int(max_episode_seconds * self.fps) + 1)
    ...
    def _is_empty_dataset_skeleton(self) -> bool:
        """Return whether the root only contains zero-count LeRobot metadata."""
        root = Path(self.dataset_root)
        info_path = root / "meta" / "info.json"
        if not root.is_dir() or root.is_symlink() or not info_path.is_file():
            return False

        files = {path.relative_to(root) for path in root.rglob("*") if path.is_file()}
        if files != {Path("meta/info.json")}:
            return False
        ...
        try:
            ...
            return False
        return all(int(info.get(field, -1)) == 0 for field in ("total_episodes", "total_frames", "total_tasks"))

    def _archive_empty_dataset_skeleton(self) -> Path:
        """Move an interrupted zero-episode initialization aside for inspection."""
        root = Path(self.dataset_root)
        archive = root.with_name(f"{root.name}.incomplete-{time.time_ns()}")
        root.rename(archive)
        return archive
    ...
    def init_dataset(self):
        if self.check_dataset_exists():
            if self._is_empty_dataset_skeleton():
                archive = self._archive_empty_dataset_skeleton()
                print(
                    "[WARNING]: Archived interrupted zero-episode dataset initialization "
                    f"at {archive}; creating a clean local dataset."
                )
            else:
                try:
                    self._init_existing_dataset()
                    self.initial_total_episodes = int(self.dataset.meta.total_episodes)
                    return
    ...
    def total_episodes(self) -> int:
        """Episodes already on disk plus episodes completed in this process."""
        return self.initial_total_episodes + self.num_recorded_episodes
    ...
    def save_episode(self, event: Event):
        if event.event_name == self.STOP_RECORDING_EVENT:
            if self.current_frame == 0 or self.action_buffers_tensor is None:
                print("[WARNING]: Empty recording ignored; no episode was queued.")
                return
    ...
    def async_episode_processor(self):
        while not self.episode_processor_stop_event.is_set():
            ...
            try:
                ...
                print(f"[INFO]: Episode {self.total_episodes} saved.")
    ...
    def wait_until_idle(self) -> None:
        """Wait for all queued episodes to finish encoding and committing."""
        self.episode_queue.join()

    def close(self) -> None:
        """Flush queued episodes and stop the processor thread."""
        self.wait_until_idle()
        self.episode_processor_stop_event.set()
        self.episode_processor_thread.join(timeout=5.0)
        self.episode_processor_thread = None
        print("[INFO]: Recorder closed.")
    ...
    def __del__(self):
        thread = getattr(self, "episode_processor_thread", None)
